...
def main():
    ...
    with sidebut2:
        if st.button('Upload a model'):
            ...
            with tab_2:
                st.header("Data Predictions and Metrics")
                st.write('Predictionsss  Belowwwwww')
                make_predictions(clean_df,key_dict,test_size,target,model)   
                st.session_state.trained = True
    ...
    if st.sidebar.button('Save the model'):
            if st.session_state.trained:
                ...
                save_path = st.text_input('Enter the filename to save the model', 'trained_model.pkl')
